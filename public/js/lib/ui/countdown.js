...
define(["common/utils"], function(Utils) {
    ...
    var target = 'June 12, 2012',
        ...
        // 9-0
        initialPosCountdown = [-4728, -4202, -3677, -3152, -2626, -2101, -1575, -1050, -525, 0],
        // 5-0 (first minute and second digit)
        initialMidPosCountdown = [-2626, -2101, -1575, -1050, -525, 0],
        // 2-0 (first hour digit)
        initialSmallPosCountdown = [-1050, -525, 0],
        classNamesCountdown = ['heures', 'minutes', 'secondes'],
        idNamesCountdown = ['h', 'm', 's'],
        animationFramesCountdown = 5,
        frameShiftCountdown = 88;



    theDiffStringCountdown = Utils.format(new Date(), "isoTime", false);
    ...
    return {
        initialDigitCheckCountdown: function(initial) {
            // Creates the html
            var a = initial.split(':');
            for (var i = 0, c = a.length; i < c; i++) {
                if (a[i].length < 2) a[i] = '0' + a[i];
        ...
        doCountCountdown: function() {
            var now = new Date();

            var beforeDate = now - 1000;
            var y = Utils.format(now, "isoTime", false);
            var x = Utils.format(beforeDate, "isoTime", false);
