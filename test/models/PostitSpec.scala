...
class PostitSpec extends Specification {
  ...
  "A Postit" should {
    ...
    "be created" in {
      ...
      var result = PostIt.create(Option(new PostIt(null, "title", PostItType.Important
        .toString, "contenu", Option(true), Option(tmp)))) mustEqual true
      Redis.disconnect()
      result
    }
